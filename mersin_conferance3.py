# =============================================================================
# STEP 9 — Enhanced Visual: 1970–2021 with gaps for zero-paper years
# Reads  : digital_convergence_data.csv  (produced by mersin_conferance1.py)
# Outputs: digital_convergence_trend_1970_2021_gapped.png
# =============================================================================
...
OUT_DIR    = os.path.join(SCRIPT_DIR, "3.çalıştırma_outputları")
...
# the year is marked as a statistical outlier (isolated dot, different colour).
MIN_N_FOR_LINE = 17

# Years manually flagged as false positives — shown as hollow markers + annotated,
# excluded from the main connected line regardless of n.
FALSE_POSITIVE_YEARS = {1972, 2007}
...
# ── Annotate confirmed false-positive years ──────────────────────────────────
# 1972: X marker sits near y≈0 on ax2 (rate=50% is off-scale).
# Annotate on ax1 so arrow tip can hit the visible bar top.
ax1.annotate(
    "confirmed false positive\nn=2, digital=1, rate=50%",
    xy=(1972, 2),                    # arrow tip → top of the 1972 bar
    xytext=(1975, 32),
    fontsize=8, color="dimgray",
    arrowprops=dict(arrowstyle="->", color="dimgray", lw=1.2,
                    connectionstyle="arc3,rad=0.35",
                    relpos=(0.0, 0.5)),
    bbox=dict(boxstyle="round,pad=0.3", fc="lightyellow", ec="dimgray", alpha=0.9),
)

# 2007: X marker sits at (2007, 2.5%) on ax2.
ax2.annotate(
...
ax1.set_title(
    ...
    f"({YEAR_FROM}–{YEAR_TO})\n"
    f"[Gaps = 0 publications or n < {MIN_N_FOR_LINE}; ○ = low sample (n<{MIN_N_FOR_LINE}); ✕ = confirmed false positive]",
    fontsize=12, pad=10,
)

tick_years = [y for y in all_years if y % 5 == 0]
if all_years[0] not in tick_years:
    tick_years = [all_years[0]] + tick_years
